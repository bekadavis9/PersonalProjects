...
The offsets in the long file are double those of the int. Also, the suffixes in the int file are l (`movl`) and the suffixes in the long file are q (`movq`).
...
The swap.s file uses different registers in the main function. Where swap_int.s uses register %eax, swap.s uses registers %rax and %rdx. It also uses registers %rsi and %rdi. Furthermore, the swap function uses more memory with greater offsets to store addresses of ints rather than just the ints themselves. 
...
The assembly for the dynamic array does not have offsets from the addresses like the static array does. 
...
Both `je` and `jmp` statements are implemented in my goto.s. The `je` command is exectuted when my code checks to see if the input number is equal to 3. The `jmp` statement is excuted in the `else` statement of goto.c.
...
The code for the condition statement is at the end. 
...
The assembly for a switch statement is very long. Each case from the switch has its own label with (in my case) 4 commands, including a `jmp` statement. The main function is pretty optimal, but it seems to have a lot more `movl` and `addl` statements than necessary.  
...
`popq` is called at the end of the add function, where `sum` is returned in the C code. 
