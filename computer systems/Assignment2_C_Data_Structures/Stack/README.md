...
Stacks are very important in computer science. In programming specifically, the execution thread follows a stack architecture among functions. An understanding of this heirarchy is important for a good programmer, as code can be optimized to take advantage of this stack.

Furthermore, like queues, stacks provide an alternative to memory allocation. Dynamic allocation allows the size of an object to be determined at run-time. This saves some time for the compiler and saves space in memory by avoiding unnecessary allocation.

A stack should be used in any situation where last-in-first-out (LiFo) functionality is desired. In many situations they can be used in lieu of arrays, but not always. LiFo sometimes implies a dependency among elements, where the most recent element needs the previously added for a calculation of some sort. In such a situation, stacks are ideal. 
