...
// want to create a queue the size of the given capacity
queue_t* create_queue(unsigned int _capacity){ 
	queue_t* q = (queue_t*)malloc(sizeof(queue_t)*_capacity); //create queue space
	q->front = NULL;
	q->back = NULL;
...
int queue_empty(queue_t* q){
	if(&(q->front) == NULL) //no front data, queue is empty
		return 1;
	else
		return 0;
}
...
int queue_full(queue_t* q){
	if(q->size == q->capacity) //number of elements enqued same as total possible elements
	{
		printf("size: %d\n", q->size);
		printf("capacity: %d\n", q->capacity);
		return 1;
...
int queue_enqueue(queue_t* q, int item){
	if(queue_full(q) == 1) //check to see if queue is full
		return -1;
	else
	{
		q->front = (q->front + 1) % q->capacity; //move pointer position
		q->front = item; //add data
		q->size += 1;
		return 0;
	}
...
int queue_dequeue(queue_t *q){
	if(queue_empty(q) == 1) //check to see if queue is empty
		exit(1); //error
	else
	{
		q->back = NULL; //remove data
		q->back = (q->back + 1) % q->capacity; //move the position of the pointer
		q->size -= 1;
		return q->front;
	}
}
